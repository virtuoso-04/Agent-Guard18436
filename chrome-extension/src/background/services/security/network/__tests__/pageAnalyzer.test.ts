...
describe('Phase 3: Phishing Page Analyzer', () => {
  it('Password form on HTTP page → critical risk', async () => {
    const mockState = {
      selectorMap: new Map([[1, { tagName: 'input', attributes: { type: 'password' } } as any]]),
  ...
  it('Urgency language in text', async () => {
    const mockState = {
      selectorMap: new Map([[1, { tagName: 'div', text: 'Your account is suspended! Verify immediately.' } as any]]),
    } as unknown as BrowserState;
  ...
  it('Suspicious form target', async () => {
    const mockState = {
      selectorMap: new Map([[1, { tagName: 'form', attributes: { action: 'https://attacker.com/collect' } } as any]]),
    } as unknown as BrowserState;
